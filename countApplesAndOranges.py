...
* When a fruit falls from its tree, it lands d units of distance from its tree of origin along the x-axis. *A negative value of d means the fruit fell d units to the 
...
apples=[2,3,-4] units distance from a, and oranges=[3,-2,-4] units distance. Adding each apple distance to the position of the tree, they land at [4+2, 4+3, 4+(-4)] =[6,7,0].
...
i=0
while i<len(apples):
    #applesfall.append(apples[i]+a) 
    if (apples[i]+a)>=s and (apples[i]+a)<=t:
        applesfall+=1 
    i+=1
i=0
while i<len(oranges):
    #orangesfall.append(oranges[i]-b)
